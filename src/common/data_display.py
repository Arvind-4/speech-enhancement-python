...
def make_plot_phase(stft_phase, sample_rate, hop_length_fft):
    ...
    plt.title(
        title.format(
            hop_length_fft, stft_phase.shape[1], stft_phase.shape[0], stft_phase.shape
        )
    )
    return
...
def make_3plots_spec_voice_noise(
    stftvoicenoise_mag_db,
    stftnoise_mag_db,
    stftvoice_mag_db,
    sample_rate,
    hop_length_fft,
...
def make_3plots_phase_voice_noise(
    stftvoicenoise_phase, stftnoise_phase, stftvoice_phase, sample_rate, hop_length_fft
):
    plt.figure(figsize=(8, 12))
    plt.subplot(3, 1, 1)
    plt.title("Phase (radian) voice + noise")
    librosa.display.specshow(
        np.angle(stftvoicenoise_phase),
        x_axis="time",
        y_axis="linear",
        sr=sample_rate,
        hop_length=hop_length_fft,
...
def make_3plots_timeseries_voice_noise(
    clipvoicenoise, clipnoise, clipvoice, sample_rate
):
